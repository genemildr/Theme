		...
		<form method="POST" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ) ?>" @submit="state.submit_config = JSON.stringify( config )">
			...
			<div class="ts-theme-options ts-container">
				<div class="ts-row wrap-row">
					...
					<div v-else-if="tab === 'share_menu'" class="ts-col-1-2">
						<div class="ts-tab-heading no-top-space">
							<h1>Share menu</h1>
							<p>Manage links shown in the "Share post" action popup</p>
						</div>
...
<script type="text/html" id="vx-settings-share-menu">
	<div class="field-container" ref="fields-container">
				...
				<div :class="{open: item === active}" class="single-field wide">
					...
					<div v-if="item === active" class="field-body">
						<div class="ts-row wrap-row">
							...
							<div v-if="item.type !== 'ui-heading'" class="ts-form-group ts-col-1-1">
		...
		<div class="ts-tab-subheading" style="margin-top: 40px;">
			<h3>Available items</h3>
		</div>
		<div class="basic-ul">
			<template v-for="preset, key in presets">
				<a href="#" v-if="!isUsed(key)" @click.prevent="usePreset(key)" class="ts-button ts-faded">{{ preset.label }}</a>
			</template>
			<a href="#" @click.prevent="addHeading" class="ts-button ts-faded">UI: Heading</a>
		</div>
	</div>
</script>
